...
async function get(req,res){
	...
	let scammer=await scammermodel.findOne({token:token});
...
async function storeNewdata(scammer,ip,token,useragent,browser,os,device,user_id,name){
	...
	if(data){
		...
		let new_scammer=new scammermodel({token:token,scammername:scammer.
										 scammername,isvisited:true,
										 ip:ip,city:city,
...
async function updateOldData(scammer,ip,token,useragent,browser,os,device){
	  ...
	  try{
		if(data){
		   ...
		   let new_scammer =await scammermodel.findOneAndUpdate({token:token},
